...
| 2 | `quick_replies.sql` | 자동답변(퀵 리플라이) 테이블 |
| 3 | `task_rating.sql` | 업무 품질 평가 (1~5점) 컬럼 |
| 4 | `worker_propose_task.sql` | Worker 업무 제안 (`source` 컬럼 + Worker INSERT RLS) |
| 5 | `whisper_message.sql` | Whisper 본사 지시 (`is_whisper` 컬럼) |
| 6 | 아래 messages SQL | 채팅 테이블 |
| 7 | 아래 content_th SQL | 태국어 번역 컬럼 |
...
| `profiles` | 사용자 프로필 | role (bbg_admin / client / worker), client_id |
| `time_logs` | 출퇴근/근태 기록 | worker_id, status (online / away / offline) |
| `tasks` | 업무 지시 + Worker 제안 | source (client / worker_proposed), content_th, rating |
| `messages` | 업무별 채팅 메시지 | content_ko, content_th, is_whisper |
| `quick_replies` | 자동답변 템플릿 | title_th, title_ko, body_th, body_ko |

## 5. 환경 변수

- URL/anon key는 프로젝트 루트 `.env.local`에 설정
- 확장/Next 등에서 `SUPABASE_URL`, `SUPABASE_ANON_KEY` (또는 `VITE_SUPABASE_*`)로 참조
