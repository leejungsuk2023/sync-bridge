...
const MonthlyReport = dynamic(() => import('@/components/MonthlyReport'), { ssr: false });
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (!profileData || (profileData.role !== 'bbg_admin' && profileData.role !== 'client' && profileData.role !== 'hospital' && profileData.role !== 'staff')) {
        router.push('/app');
        return;
      }
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
        <div className="max-w-[1440px] mx-auto px-3 sm:px-6 h-14 sm:h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold text-slate-900">월간 보고서</h1>
          </div>
        </div>
      </header>
