...
export async function POST(req: NextRequest) {
  ...
  try {
    console.log(`[SuggestFeedback] Recording feedback for suggestion ${suggestion_id}, selected_index=${selected_index}, was_edited=${was_edited}`);
    ...
    if (error) {
      console.error('[SuggestFeedback] DB error:', error.message);
      return withCors(NextResponse.json({ error: error.message }, { status: 500 }));
    }

    console.log(`[SuggestFeedback] Feedback recorded for suggestion ${suggestion_id}`);
    return withCors(NextResponse.json({ ok: true }));
  } catch (err: any) {
    console.error('[SuggestFeedback] Error:', err?.message || err);
