...
- 처방 알림 기능 추가 필요: `GOOGLE_SERVICE_ACCOUNT_BASE64` (Google Sheets 접근용 서비스 계정 JSON, Base64 인코딩), `GOOGLE_SHEET_ID` (진단 결과 Google Sheet ID)
...
client-web/app/api/channels/     # LINE/Facebook 채널 설정 API (line/facebook/conversations/messages/read/reply/status/suggest-feedback/suggest-reply/upload) + chatbot-toggle (채널별 챗봇 ON/OFF, bbg_admin/staff 전용)
client-web/app/api/messaging/    # 다이렉트 메시징 API (config, generate, upload-csv) + auto-reply (Korean Diet AI 챗봇 엔진, LINE webhook 수신 후 자동 답변. JSON 응답 포맷으로 survey_name 추출 후 customers 테이블에 저장) + suggest-reply (채널 대화용 AI 답변 추천, hospital_procedures/promotions/info 컨텍스트 포함) + prescription-notify (Google Sheet 진단 결과 읽기 → LINE 자동 안내 발송 Cron, 5분마다)
client-web/app/api/rag/          # RAG 검색 인덱싱 API (index: Zendesk 전체 병원 케이스 일 1회, index-koreandiet: Korean Diet 케이스 일 1회, search: 벡터 유사도 검색)
...
client-web/vercel.json           # Vercel Cron 스케줄 (zendesk/cron 일 2회, followup-summary 일 4회, poll 일 4회, rag/index 일 1회 02:00 UTC, rag/index-koreandiet 일 1회 03:00 UTC, followup-check 일 1회 01:30 UTC, prescription-notify 5분마다)
...
client-web/components/
  ...
  MessagingLayout.tsx            # LINE/Facebook 다이렉트 메시징 오케스트레이터 (워커/스태프 상담 탭) — 상단 채널별 챗봇 토글 바 포함
  ConversationList.tsx           # 다이렉트 메시징 대화 목록 패널
  MessagePanel.tsx               # 다이렉트 메시징 채팅 패널 — 대화별 챗봇 개별 토글 포함
...
supabase/chatbot_channel_toggle.sql # messaging_channels.chatbot_enabled 컬럼 추가 (채널별 챗봇 토글)
supabase/chatbot_toggle.sql      # channel_conversations.chatbot_enabled 컬럼 추가 (대화별 챗봇 토글)
supabase/rag_case_index.sql      # RAG 검색 테이블 (case_index: 벡터 임베딩 768dim, case_conversations: 원본 대화 보관) + pgvector 활성화
supabase/survey_name.sql         # customers 테이블 survey_name 컬럼 추가 (처방 알림 매칭용) + 인덱스
supabase/migrations/20260314165649_survey_name.sql # Supabase CLI 마이그레이션 버전 (survey_name.sql 동일 내용)
```
...
- 임베딩 모델: `gemini-embedding-001` (768 dims) — `text-embedding-004`에서 마이그레이션 완료
- RAG 테이블: `case_index` (벡터 검색), `case_conversations` (원본 대화)
- `case_index.embedding_model` 컬럼으로 모델 버전 추적
- **Korean Diet 전용 인덱서:** `/api/rag/index-koreandiet` — Zendesk 티켓에서 `koreandiet` 태그 필터링, 일 1회 Cron (03:00 UTC)
- **전체 인덱서:** `/api/rag/index` — 전체 병원 케이스, 일 1회 Cron (02:00 UTC)
- **검색:** `/api/rag/search` — 벡터 유사도 검색 (hospital_name 필터 가능)

## Korean Diet AI 챗봇

- **엔진:** `/api/messaging/auto-reply` — LINE webhook 수신 후 자동 답변 생성 (POST, CRON_SECRET 불필요, channel_type 기반 라우팅)
- **응답 포맷:** Gemini가 JSON `{"reply": "...", "survey_name": "..."}` 형식으로 응답. 파싱 실패 시 plain text 폴백. `survey_name`이 추출되면 `customers.survey_name`에 저장 (처방 알림 매칭용)
- **판매 플로우 3.5단계:** 고객이 건강설문 작성 후 챗봇이 설문에 사용한 성명 확인 요청 → survey_name 저장
- **제품 지식:** `hospital_procedures`, `hospital_promotions`, `hospital_info` 테이블에서 RAG 필터링 (`.eq('hospital_name', 'Korean Diet')`)
- **가격:** 1box 99,000원 / 2box 149,000원 / 4box 249,000원 (THB 환산 포함)
- **Anti-hallucination:** 프롬프트에 정확한 가격 외 답변 금지 규칙 포함
- **채널 토글 우선순위:** 채널 ON → 모든 대화 자동 답변; 채널 OFF → 대화별 개별 토글 적용

## 처방 알림 시스템 (Prescription Notify)

- **엔진:** `/api/messaging/prescription-notify` — Google Sheet에서 의사 진단 결과를 읽어 LINE으로 자동 안내 발송 (GET/POST, CRON_SECRET 인증)
- **Cron 스케줄:** `*/5 * * * *` (5분마다 실행, Vercel Pro 플랜)
- **매칭 로직:** Google Sheet 환자 → LINE 대화 3단계 퍼지 매칭
  1. `customers.survey_name` 정확/부분 일치 (가장 신뢰도 높음)
  2. LINE 표시이름(`display_name`) 정확/부분 일치 (폴백)
  3. 이름 첫 부분 `display_name` 부분 일치 (최후 수단)
- **처리 조건:** diagnosis(1단계/2단계) + consent=yes + status가 상담중 또는 비어있는 행만 처리
- **FB 채널 스킵:** channel='FB'인 행은 건너뜀 (LINE 전용)
- **시트 상태 업데이트:** 성공 시 O열에 `자동안내완료`, 매칭 실패 시 `매칭불가` 기록
- **알림 메시지:** 태국어 이중언어 메시지 (진단 단계 + 가격 + 은행 계좌 정보 포함)
- **필수 환경 변수:** `GOOGLE_SERVICE_ACCOUNT_BASE64`, `GOOGLE_SHEET_ID`
- **npm 의존성:** `googleapis` (google-auth-library 포함)

## 챗봇 토글 계층

| 토글 레벨 | 저장 위치 | 관리 API | 설명 |
|-----------|-----------|---------|------|
| 채널 레벨 | `messaging_channels.chatbot_enabled` | `PUT /api/channels/chatbot-toggle` | 채널 전체 ON/OFF. 채널 ON이면 대화별 설정 무시 |
| 대화 레벨 | `channel_conversations.chatbot_enabled` | `MessagePanel.tsx` 내 토글 | 채널 OFF일 때만 유효한 개별 대화 토글 |
