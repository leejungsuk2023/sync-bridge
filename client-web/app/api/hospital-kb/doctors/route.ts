...
// Verify that a hospital_id belongs to a client's own hospital
...
// GET /api/hospital-kb/doctors?hospital_id=xxx
export async function GET(req: NextRequest) {
  ...
  if (error) {
    console.error('[HospitalKB] Error fetching doctors:', error.message);
    return withCors(NextResponse.json({ error: error.message }, { status: 500 }));
  }

  return withCors(NextResponse.json({ doctors: data }));
}

// POST /api/hospital-kb/doctors — create doctor (bbg_admin or client for own hospital)
export async function POST(req: NextRequest) {
  ...
  if (!body || !body.hospital_id || !body.name_ko) {
    return withCors(NextResponse.json({ error: 'hospital_id and name_ko required' }, { status: 400 }));
  }
  ...
  if (error) {
    console.error('[HospitalKB] Error creating doctor:', error.message);
    return withCors(NextResponse.json({ error: error.message }, { status: 500 }));
  }

  return withCors(NextResponse.json({ doctor: data }, { status: 201 }));
}

// PUT /api/hospital-kb/doctors?id=xxx — update doctor (bbg_admin or client for own hospital)
export async function PUT(req: NextRequest) {
  ...
  if (userInfo.role === 'client') {
    const { data: existing } = await supabaseAdmin
      ...
      .single();
    if (!existing) {
      return withCors(NextResponse.json({ error: 'Doctor not found' }, { status: 404 }));
    }
  ...
  if (error) {
    console.error('[HospitalKB] Error updating doctor:', error.message);
    return withCors(NextResponse.json({ error: error.message }, { status: 500 }));
  }

  return withCors(NextResponse.json({ doctor: data }));
}

// DELETE /api/hospital-kb/doctors?id=xxx — delete doctor (bbg_admin only)
export async function DELETE(req: NextRequest) {
  ...
  if (error) {
    console.error('[HospitalKB] Error deleting doctor:', error.message);
    return withCors(NextResponse.json({ error: error.message }, { status: 500 }));
  }
