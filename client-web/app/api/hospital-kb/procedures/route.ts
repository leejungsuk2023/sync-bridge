...
// GET /api/hospital-kb/procedures?hospital_id=xxx[&category=눈]
export async function GET(req: NextRequest) {
  ...
  if (error) {
    console.error('[HospitalKB] Error fetching procedures:', error.message);
  ...
  return withCors(NextResponse.json({ procedures: data, by_category: byCategory }));
}

// POST /api/hospital-kb/procedures — create procedure (bbg_admin or client for own hospital)
export async function POST(req: NextRequest) {
  ...
  if (!body || !body.hospital_id || !body.category || !body.name_ko) {
    return withCors(NextResponse.json({ error: 'hospital_id, category, and name_ko required' }, { status: 400 }));
  }
  ...
  if (error) {
    console.error('[HospitalKB] Error creating procedure:', error.message);
    return withCors(NextResponse.json({ error: error.message }, { status: 500 }));
  }

  return withCors(NextResponse.json({ procedure: data }, { status: 201 }));
}

// PUT /api/hospital-kb/procedures?id=xxx — update procedure (bbg_admin or client for own hospital)
export async function PUT(req: NextRequest) {
  ...
  if (userInfo.role === 'client') {
    const { data: existing } = await supabaseAdmin
      ...
      .single();
    if (!existing) {
      return withCors(NextResponse.json({ error: 'Procedure not found' }, { status: 404 }));
    }
  ...
  if (error) {
    console.error('[HospitalKB] Error updating procedure:', error.message);
    return withCors(NextResponse.json({ error: error.message }, { status: 500 }));
  }

  return withCors(NextResponse.json({ procedure: data }));
}

// DELETE /api/hospital-kb/procedures?id=xxx — delete procedure (bbg_admin only)
export async function DELETE(req: NextRequest) {
  ...
  if (error) {
    console.error('[HospitalKB] Error deleting procedure:', error.message);
    return withCors(NextResponse.json({ error: error.message }, { status: 500 }));
  }
